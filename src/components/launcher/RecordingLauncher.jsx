...
export function RecordingLauncher() {
  ...
  return (
    <div className="fixed inset-0 z-[9990] flex items-center justify-center p-5">
      {/* Backdrop */}
      <div className="absolute inset-0 bg-black/25 backdrop-blur-[6px] anim-fade-in"
        ...
        <div className="surface-modal overflow-hidden">
          ...
          <div className="flex items-center justify-between px-6 py-5"
            ...
            <button onClick={closeLauncher}
              ...
              onMouseEnter={(e) => { e.currentTarget.style.background = 'var(--bg-soft)'; e.currentTarget.style.color = 'var(--text-1)'; }}
          ...
          <div className="p-6 space-y-6">
              ...
              <div className="grid grid-cols-3 gap-2.5">
                {MODES.map(({ key, label, icon: Icon }) => (
                  <button key={key} onClick={() => setMode(key)}
                    ...
                    style={{
                      background: mode === key ? 'var(--purple-soft)' : 'var(--bg-soft)',
                      borderColor: mode === key ? 'var(--purple-border)' : 'var(--border)',
              ...
              {(mode === RECORDING_MODE.SCREEN || mode === RECORDING_MODE.SCREEN_CAMERA) && (
                <OptionRow>
                  <Toggle checked={systemAudioEnabled} onChange={toggleSystemAudio}
                    ...
                    icon={systemAudioEnabled ? <Volume2 size={15} /> : <VolumeX size={15} />}
                    label="System audio"
            ...
            {/* ── Error ── */}
            {error && (
              <div className="rounded-[14px] border px-4 py-3 text-[13px] anim-fade-in"
            ...
            <Button variant="primary" size="lg" className="w-full" onClick={handleStart}
              ...
              {!isRequesting && <span className="w-2.5 h-2.5 rounded-full bg-white/80" />}
              {isRequesting ? 'Requesting access…' : 'Start recording'}
            </Button>

            <p className="text-center text-[11px]" style={{ color: 'var(--text-3)' }}>
              Press <kbd className="px-1.5 py-0.5 rounded-[6px] font-mono text-[10px]"
                style={{ background: 'var(--bg-soft)', border: '1px solid var(--border)', color: 'var(--text-2)' }}>
                Esc</kbd> to cancel
            </p>
          </div>
        </div>
...
function OptionRow({ children }) {
  return (
    <div className="flex items-center px-3 py-3 rounded-[12px] transition-colors"
